 ...
 * 
 * @author Yadav
 * Do not modified below class until , you know what you are doing.
 * Below class is the replica of the JdbcConnectionRepository of spring-social.jar
 * We have overriden the addConnection method to persit the data in single go.
 */
public class SocialJdbcConnectionRepository implements ConnectionRepository {
	...
	public SocialJdbcConnectionRepository(String userId, DataSource dataSource, ConnectionFactoryLocator connectionFactoryLocator, TextEncryptor textEncryptor, String tablePrefix) {
		this.userId = userId;
...
//			Users currentUser = (Users) CommonUtil.getCurrentSession().getAttribute("user");
			...
			String accessToken = (String) fs.get(connection);
			...
			{
				if(userConnection.getUserId()== facebookProfile.getEmail())
				{
		...
		} catch (DuplicateKeyException | NoSuchFieldException | SecurityException | IllegalArgumentException | IllegalAccessException e) {
